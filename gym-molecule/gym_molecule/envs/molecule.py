...
def load_conditional(type='low'):
    if type=='low':
        ...
        with open(path, 'r') as fp:
            ...
            data = data[0:800]
    elif type=="dopamine_25":
        cwd = os.path.dirname(__file__)
        path = os.path.join(os.path.dirname(cwd), 'dataset',
                            'smiles_active_conditional_25.csv')
        import csv
        with open(path, 'r') as fp:
            reader = csv.reader(fp, delimiter=',', quotechar='"')
            # data = [[row[1], row[0], id] for id, row in enumerate(reader)]
    ...
    elif type=="dopamine_15":
        cwd = os.path.dirname(__file__)
        path = os.path.join(os.path.dirname(cwd), 'dataset',
                            'smiles_active_conditional_15.csv')
        import csv
        with open(path, 'r') as fp:
            reader = csv.reader(fp, delimiter=',', quotechar='"')
            # data = [[row[1], row[0], id] for id, row in enumerate(reader)]
                    ...
                    if self.reward_type == 'logppen':
                      ...
                      #todo: check the reward_property function  ##The control is coming here at least.
                        ...
                        reward_final +=mult_reward(self.model, self.model2, final_mol, self.scaler, self.features_scaler,
                                                        ...
                                                        self.train_args2, self.args2) + reward_qed
                    # todo: check the reward_property function  ##The control is coming here at least.
...
smile = 'NC1=CC=CC=C1COC1=CC=CC(C=CC2=CC2=C(Br)C(Br)=C2C=CC=C2F)=C1'
